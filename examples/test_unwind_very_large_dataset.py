#!/usr/bin/env python3
"""
Debug script to test unwind operations with very large datasets.

This script tests the same unwind operations with 10,000 documents to see if
performance characteristics continue to improve with even larger datasets.
"""
...
def main():
    print("=== NeoSQLite Unwind Performance Test (10,000 documents) ===")

    with neosqlite.Connection(":memory:") as conn:
        # Create collection
        products = conn["products"]

        # Insert very large test data
        print("1. Preparing test data (10,000 documents)...")
        product_docs = []
        ...
        users = conn["users"]
        user_docs = []
        for i in range(5000):  # Very large dataset
            user_docs.append(
                {
                    "_id": i + 1,
                    "name": f"User {i + 1}",
        ...
        results["$unwind (single)"] = test_unwind_execution(
            products,
            [{"$unwind": "$tags"}],
            "Single $unwind operation (10,000 documents)",
        )
        ...
        results["$unwind (multiple)"] = test_unwind_execution(
            ...
            "Multiple consecutive $unwind operations (5,000 documents)",
        )
        ...
        results["$unwind (nested)"] = test_unwind_execution(
            ...
            "Nested array $unwind operations (5,000 documents)",
        )

        # Summary
        print("\n" + "=" * 60)
        print("VERY LARGE DATASET TEST SUMMARY")
        print("=" * 60)
    ...
    print("\n=== Very Large Dataset Test Complete ===")
