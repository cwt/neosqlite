...
def compare_update_modifiers():
    ...
    with neosqlite.Connection(":memory:") as neo_conn:
        ...
        # Test $each with $push - ONLY time if MongoDB is available
        if mongo_available:
            start_neo_timing()
        try:
            try:
        ...
        # Test $position with $push - ONLY time if MongoDB is available
        if mongo_available:
            start_neo_timing()
        try:
            try:
        ...
        # Test $slice with $push - ONLY time if MongoDB is available
        if mongo_available:
            start_neo_timing()
        try:
            try:
        ...
        # Test $bit (AND operation) - ONLY time if MongoDB is available
        if mongo_available:
            start_neo_timing()
        try:
            try:
