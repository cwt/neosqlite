...
def demonstrate_quez_stats():
    ...
    with neosqlite.Connection(":memory:") as conn:
        ...
        print(f"   First document size: {len(str(results_normal[0])):,} bytes\n")
        ...
        if stats_before:
            ...
            print(f"     Raw size: {stats_before['raw_size_bytes'] / 1024:.1f} KB")
            print(f"     Compressed size: {stats_before['compressed_size_bytes'] / 1024:.1f} KB")
            if stats_before['compression_ratio_pct'] is not None:
                print(f"     Compression ratio: {stats_before['compression_ratio_pct']:.1f}%")
                savings = stats_before['raw_size_bytes'] - stats_before['compressed_size_bytes']
                print(f"     Memory savings: {savings:,} bytes ({savings / 1024:.1f} KB)")
        ...
        if stats_after:
            ...
            print(f"     Raw size: {stats_after['raw_size_bytes'] / 1024:.1f} KB")
            print(f"     Compressed size: {stats_after['compressed_size_bytes'] / 1024:.1f} KB")
            if stats_after['compression_ratio_pct'] is not None:
                print(f"     Compression ratio: {stats_after['compression_ratio_pct']:.1f}%")
        ...
        print("   • Real-time compression statistics via cursor.get_quez_stats()")
