# coding: utf-8
"""
Test cases for enhanced $unwind + $group operations with SQL optimization
"""
import neosqlite
import pytest


def test_unwind_then_group_enhanced():
    """Test $unwind followed by $group with SQL optimization"""
...
def test_unwind_group_with_match_enhanced():
    ...
    with neosqlite.Connection(":memory:") as conn:
        ...
        result.sort(key=lambda x: x["_id"])
        tags = [doc["_id"] for doc in result]
        assert tags == ["go", "javascript", "python", "rust"]


def test_unwind_group_by_other_field():
    """Test $unwind + $group by another field (not the unwound field)"""
    with neosqlite.Connection(":memory:") as conn:
        collection = conn["test_collection"]
...
def test_unwind_group_with_count_accumulator():
    ...
    with neosqlite.Connection(":memory:") as conn:
        ...
        # Test $unwind then $group with $count
        pipeline = [
            {"$unwind": "$tags"},
        ...
        # Should count occurrences of each tag
        assert len(result) == 3  # python, javascript, java

        # Check counts
        counts = {doc["_id"]: doc["total"] for doc in result}
