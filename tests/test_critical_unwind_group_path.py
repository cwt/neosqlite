...
def test_unwind_group_with_addtoset_accumulator():
    ...
    with Connection(":memory:") as conn:
        ...
        collection.insert_many([
            ...
            {"type": "fruits", "names": ["apple", "orange"]},  # apple appears twice
        ...
        pipeline = [
            ...
            {"$group": {
                "_id": "$type",
                "unique_names": {"$addToSet": "$names"}  # $addToSet accumulator
            }}
        ]
...
def test_unwind_group_with_count_accumulator():
    ...
    with Connection(":memory:") as conn:
        ...
        collection.insert_many([
            ...
            {"region": "north", "cities": ["NYC", "Philadelphia"]},  # NYC appears twice
            {"region": "south", "cities": ["Miami", "Atlanta", "NYC"]}  # NYC appears in south too
        ])
        ...
        # Verify results
        assert len(results) >= 3  # At least NYC, Boston, Miami, Atlanta, Philadelphia
...
def test_unwind_group_fallback_conditions():
    ...
    with Connection(":memory:") as conn:
        ...
        pipeline = [
            ...
            {"$group": {
                "_id": "$tags",
                "complex_calc": {"$sum": {"$multiply": ["$tags", 2]}}  # Complex expression should cause fallback
            }}
        ]
