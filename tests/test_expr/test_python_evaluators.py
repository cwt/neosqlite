...
def test_logical_operators(evaluator):
    ...
    assert (
        eval_py(
            evaluator, {"$nor": [{"$eq": ["$a", 2]}, {"$eq": ["$a", 3]}]}, doc
        )
        is True
    )
    assert (
        eval_py(
            evaluator, {"$nor": [{"$eq": ["$a", 1]}, {"$eq": ["$a", 3]}]}, doc
        )
        is False
    )
    assert (
        eval_py(
            evaluator, {"$and": [{"$eq": ["$a", 1]}, {"$literal": True}]}, doc
        )
        is True
    )
    assert (
        eval_py(
            evaluator, {"$or": [{"$eq": ["$a", 2]}, {"$literal": True}]}, doc
        )
        is True
    )
...
def test_math_operators(evaluator):
    ...
    assert (
        eval_py(
            evaluator,
            {"$sigmoid": {"input": None, "onNull": {"$literal": 1.0}}},
            doc,
        )
        == 1.0
    )
...
def test_trig_operators(evaluator):
    ...
    assert eval_py(evaluator, {"$degreesToRadians": 180}, {}) == pytest.approx(
        math.pi
    )
...
def test_conditional_operators(evaluator):
    # $cond
    assert (
        eval_py(evaluator, {"$cond": [{"$literal": True}, "yes", "no"]}, {})
        == "yes"
    )
    assert (
        eval_py(
            evaluator,
            {"$cond": {"if": {"$literal": True}, "then": "yes", "else": "no"}},
            {},
        )
        == "yes"
    )
    assert (
        eval_py(
            evaluator, {"$cond": {"if": {"$literal": False}, "then": "yes"}}, {}
        )
        is None
    )
...
def test_array_operators(evaluator):
    ...
    # N operators
    assert eval_py(evaluator, {"$firstN": {"input": "$arr", "n": 2}}, doc) == [
        1,
        2,
    ]
    assert eval_py(evaluator, {"$lastN": {"input": "$arr", "n": 2}}, doc) == [
        2,
        3,
    ]
    assert eval_py(evaluator, {"$maxN": {"input": [3, 1, 2], "n": 2}}, doc) == [
        3,
        2,
    ]
    assert eval_py(evaluator, {"$minN": {"input": [3, 1, 2], "n": 2}}, doc) == [
        1,
        2,
    ]
    ...
    assert eval_py(evaluator, {"$slice": [[1, 2, 3, 4, 5], 2, 1]}, doc) == [
        2,
        3,
    ]
...
def test_string_operators(evaluator):
    ...
    assert eval_py(evaluator, {"$split": ["a,b,c", ","]}, doc) == [
        "a",
        "b",
        "c",
    ]
...
def test_date_operators(evaluator):
    ...
    assert (
        eval_py(evaluator, {"$dayOfWeek": "$d"}, doc) == 7
    )  # Saturday (1=Sunday)
    assert eval_py(evaluator, {"$dayOfYear": "$d"}, doc) == 69
    assert (
        eval_py(evaluator, {"$isoDayOfWeek": "$d"}, doc) == 6
    )  # Saturday (1=Monday)
    ...
    # Conversion
    res_str = eval_py(
        evaluator, {"$dateToString": {"date": "$d", "format": "%Y-%m-%d"}}, doc
    )
    assert res_str == "2024-03-09"

    res_from_str = eval_py(
        evaluator,
        {"$dateFromString": {"dateString": "2024-03-09T12:00:00Z"}},
        doc,
    )
    ...
    res_from_parts = eval_py(
        evaluator, {"$dateFromParts": {"year": 2024, "month": 3, "day": 9}}, doc
    )
    assert res_from_parts.year == 2024

    res_trunc = eval_py(
        evaluator, {"$dateTrunc": {"date": "$d", "unit": "month"}}, doc
    )
...
def test_object_operators(evaluator):
    doc = {"obj": {"a": 1}}
    assert eval_py(evaluator, {"$mergeObjects": [{"x": 1}, {"y": 2}]}, doc) == {
        "x": 1,
        "y": 2,
    }
    assert (
        eval_py(evaluator, {"$getField": {"field": "a", "input": "$obj"}}, doc)
        == 1
    )
    assert eval_py(
        evaluator,
        {"$setField": {"field": "b", "input": "$obj", "value": 2}},
        doc,
    ) == {"a": 1, "b": 2}
    assert (
        eval_py(
            evaluator, {"$unsetField": {"field": "a", "input": "$obj"}}, doc
        )
        == {}
    )
    assert eval_py(evaluator, {"$objectToArray": "$obj"}, doc) == [
        {"k": "a", "v": 1}
    ]
    # $let fix
    assert (
...
def test_type_operators(evaluator):
    ...
    assert (
        eval_py(evaluator, {"$convert": {"input": "$a", "to": "int"}}, doc)
        == 123
    )
...
def test_literal_and_operands(evaluator):
    doc = {"a": 1, "$$v": 2}
    assert eval_py(evaluator, {"$literal": {"$add": [1, 2]}}, doc) == {
        "$add": [1, 2]
    }
    assert evaluator._evaluate_operand_python("$a", doc) == 1
    assert evaluator._evaluate_operand_python("$$v", doc) == 2
    assert (
        evaluator._evaluate_operand_python("$$REMOVE", doc) == REMOVE_SENTINEL
    )
...
def test_logical_operators_ext(evaluator):
    doc = {"a": 1}
    assert (
        evaluator._evaluate_logical_python("$and", [{"$eq": ["$a", 1]}], doc)
        is True
    )
    assert (
        evaluator._evaluate_logical_python("$or", [{"$eq": ["$a", 2]}], doc)
        is False
    )
...
def test_array_sort_errors(evaluator):
    ...
    assert eval_py(
        evaluator, {"$sortArray": {"input": [1, "a"], "sortBy": None}}, {}
    ) == [1, "a"]
    assert eval_py(
        evaluator,
        {"$sortArray": {"input": [{"x": 1}, {"x": "a"}], "sortBy": {"x": 1}}},
        {},
    ) == [{"x": 1}, {"x": "a"}]
