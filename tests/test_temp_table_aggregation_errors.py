...
class TestFacetStageUTF8DecodeError:
    ...
    def test_facet_with_corrupted_data_column(self):
        """
        Test $facet when temp table has non-UTF8 bytes in data column.

        This verifies that the error 'utf-8' codec can't decode byte 0xdc
        is now handled gracefully - corrupted documents are skipped with a warning.
        """
        with neosqlite.Connection(":memory:") as conn:
            coll = conn.test_facet_corrupted

            coll.insert_one({"name": "doc1", "value": 42})

            # Create a temp table with corrupted (non-UTF8) data
            # Simulating what happens when binary data isn't properly encoded
            corrupted_bytes = b"\xdc\xab\xcd\xef"  # Invalid UTF-8 sequence
            conn.db.execute(
                "CREATE TEMP TABLE corrupted_temp AS SELECT id, _id, ? as data FROM test_facet_corrupted",
                (corrupted_bytes,),
            )
            ...
            # Should handle corrupted data gracefully without raising errors
            # The corrupted document should be skipped
            result_table = processor._process_facet_stage(
                lambda spec, query: "dummy",
                "corrupted_temp",
                {"results": [{"$project": {"name": 1}}]},
            )

            # Should have created a result table (even if empty due to skipped docs)
            assert result_table is not None

    def test_facet_with_raw_binary_not_json_encoded(self):
        """
        Test $facet when data contains raw Binary bytes instead of JSON-encoded dict.

        This verifies that the scenario where Binary objects aren't properly encoded
        is now handled gracefully - corrupted documents are skipped with a warning.
        """
        with neosqlite.Connection(":memory:") as conn:
            coll = conn.test_facet_raw_binary

            coll.insert_one({"name": "test"})

            # Create a table where data column has raw bytes (not valid JSON)
            # This simulates the bug where Binary objects aren't properly encoded
            raw_binary = bytes([0xDC, 0x80, 0xFF, 0x00, 0x81])
            conn.db.execute(
                "CREATE TEMP TABLE raw_binary_table (id INTEGER PRIMARY KEY, _id TEXT, data BLOB)"
            )
            conn.db.execute(
                "INSERT INTO raw_binary_table (id, _id, data) VALUES (1, 'test1', ?)",
                (raw_binary,),
            )
            ...
            # Should succeed - corrupted document skipped, empty result created
            assert result_table is not None
