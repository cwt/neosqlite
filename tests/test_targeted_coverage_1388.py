...
def test_unwind_group_with_group_by_different_field():
    ...
    with Connection(":memory:") as conn:
        ...
        assert type_data["electronics"]["product_count"] == 4  # 4 products total
    ...
    with Connection(":memory:") as conn:
        ...
        pipeline = [
            ...
            {"$group": {
                ...
                "unique_scores": {"$addToSet": "$scores"} # addToSet accumulator
            }}
        ]
        ...
        assert len(team_data["A"]["unique_scores"]) >= 1  # At least some unique scores
...
def test_unwind_group_edge_case_invalid_accumulator():
    ...
    with Connection(":memory:") as conn:
        ...
        pipeline = [
            ...
            {"$group": {
                "_id": "$tags",
                "count": {"$invalidOp": 1}  # Invalid operator should cause fallback
            }}
        ]
    ...
    with Connection(":memory:") as conn:
        ...
        pipeline = [
            ...
            {"$group": {
                ...
                "stats": {
                    "$sum": {"$multiply": ["$values", 2]}  # Complex expression
                }
            }}
        ]
