    ...
    with Connection(":memory:") as conn:
        ...
        test_documents = [
            {"name": "Alice", "age": 30, "department": "Engineering", "salary": 75000, 
             "skills": ["python", "javascript"], "active": True},
            {"name": "Bob", "age": 25, "department": "Marketing", "salary": 60000, 
             "skills": ["design", "copywriting"], "active": True},
            {"name": "Charlie", "age": 35, "department": "Engineering", "salary": 85000, 
             "skills": ["python", "go", "rust"], "active": False},
            {"name": "Diana", "age": 28, "department": "Engineering", "salary": 70000, 
             "skills": ["javascript", "typescript"], "active": True},
            {"name": "Eve", "age": 32, "department": "Marketing", "salary": 65000, 
             "skills": ["analytics", "design"], "active": True}
        ]
        
        collection.insert_many(test_documents)
        
        # Test 1: Simple $match
        results = list(collection.aggregate([{"$match": {"department": "Engineering"}}]))
        assert len(results) == 3
        
        # Test 2: $match with operators
        results = list(collection.aggregate([{"$match": {"age": {"$gte": 30}}}]))
        assert len(results) == 3
        
        # Test 3: $match with $in
        results = list(collection.aggregate([{"$match": {"department": {"$in": ["Engineering", "Marketing"]}}}]))
        ...
        # Test 5: $limit
        results = list(collection.aggregate([{"$sort": {"age": 1}}, {"$limit": 2}]))
